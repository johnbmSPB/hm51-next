...
export async function POST(request: Request) {
  try {
    ...
    const gamerTeamId = String(data.gamerTeamId || "").trim();
    ...
    if (!gamerTeamId) {
      return Response.json(
        { result: false, error: "GAMER_TEAM_ID не передан" },
        { status: 400 }
      );
    }
    ...
    body.append("GAMER_TEAM_ID", gamerTeamId);

    const response = await fetch("https://itandsports.ru/gamer-team/get_photo_only.php", {
      method: "POST",
      headers: {
    ...
    if (!response.ok || buffer.length === 0) {
      return Response.json(
        { result: false, error: "Фото игрока не найдено" },
        { status: 404 }
      );
    }
    ...
    if (textStart.includes('"result":false') || textStart.trim() === "0") {
      return Response.json(
        { result: false, error: "Фото игрока отсутствует" },
        { status: 404 }
      );
    }
    ...
    if (!image) {
      return Response.json(
        {
          result: false,
          error: "Сервер вернул фото в неизвестном формате",
          preview: textStart,
        },
        { status: 500 }
      );
