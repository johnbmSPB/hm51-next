...
export default function MenuPolicyPage() {
  const policyPath = path.join(process.cwd(), "app", "policy", "policy.txt");
  const policyText = fs.existsSync(policyPath)
    ? fs.readFileSync(policyPath, "utf8")
    : "Политика конфиденциальности не найдена.";

  return (
    <main className="min-h-screen bg-[#07110c] px-5 pb-28 pt-8 text-white">
      <div className="mx-auto max-w-md">
        <header className="flex items-end justify-between gap-4">
          <div>
            <p className="text-sm text-white/40">ХМ 5.1</p>
            <h1 className="text-3xl font-black">Политика</h1>
          </div>
