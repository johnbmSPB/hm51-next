...
function messageFromPush(payload: any): ChatMessage | null {
  const data = payload?.data || payload || {};
  const notification = payload?.notification || payload?.webpush?.notification || {};

  const teamId = text(
    pick(data, ["team", "TEAM", "team_id", "TEAM_ID"]) ||
      pick(payload, ["team", "TEAM", "team_id", "TEAM_ID"])
  );

  const body = decodeSafe(
    text(
      pick(data, ["text", "TEXT", "message", "MESSAGE", "body", "BODY"]) ||
        notification.body ||
        payload?.body
    )
  );
...
export default function ClientChatStable() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(importStoredPushes, 2000);
  ...
  useEffect(() => {
    ...
    function onSwMessage(event: MessageEvent) {
      ...
      const message = messageFromPush(event.data.payload);
      if (!message) return;
  ...
  async function importStoredPushes() {
    try {
      ...
      const incoming = records
        .map((record) => messageFromPush(record.payload || record.message || record))
