...
export default function ChatActions({ chat }: { chat: Controller }) {
  ...
  if (confirmingDelete) {
    const title = deleteForEveryone
      ? "Удалить сообщение у всех?"
      : queued
        ? "Отменить отправку?"
        : "Удалить сообщение с этого устройства?";

    const description = deleteForEveryone
      ? "Сообщение будет удалено у всех участников командного чата."
      : queued
        ? "Сообщение будет удалено из очереди и больше не отправится."
        : "Сообщение будет скрыто только на этом устройстве.";

    return (
      <div
        className="fixed inset-0 z-[90] flex items-end bg-black/55 px-3 pb-[max(16px,env(safe-area-inset-bottom))]"
        onClick={close}
      >
        <div className="mx-auto w-full max-w-md" onClick={(event) => event.stopPropagation()}>
          <div className="rounded-[28px] bg-[#202622] p-5 text-white shadow-2xl shadow-black/50">
            ...
            <p className="mt-2 text-sm font-semibold leading-5 text-white/55">{description}</p>
            <div className="mt-5 flex gap-2">
              ...
              >
                {queued ? "Отменить отправку" : "Удалить"}
              </button>
              <button
                type="button"
      ...
      <div className="mx-auto w-full max-w-md" onClick={(event) => event.stopPropagation()}>
        <div className="overflow-hidden rounded-[28px] bg-[#202622]">
          ...
          <button
            ...
            className="h-14 w-full px-5 text-left font-black text-red-300"
          >
            {deleteForEveryone
              ? "Удалить у всех"
              : queued
