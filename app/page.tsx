...
const plans = [
  ["Стартовый", "990 ₽ / месяц", "Для одной команды", ["Одна хоккейная команда", "Календарь игр и тренировок", "Состав и посещаемость", "Командный чат", "Базовый финансовый учёт", "Техническая поддержка"]],
  ["Командный Pro", "1 990 ₽ / месяц", "Рекомендуемый", ["Всё из тарифа «Стартовый»", "Полный финансовый модуль", "Расширенные отчёты и статистика", "Управление заявками и ролями", "Приоритетная техническая поддержка", "Скидка 20% при оплате за год"]],
  ["Клубный", "от 4 990 ₽ / месяц", "Для нескольких команд", ["До 5 команд в одном кабинете", "Единое управление клубом", "Общие справочники и стадионы", "Сводная аналитика по командам", "Персональное внедрение", "Приоритетная поддержка"]],
];
...
export default function LandingPage() {
  return (
    <main className="landing-page">
      ...
      <section className="section pricing-section" id="pricing"><div className="section-heading"><p className="eyebrow">Тарифы Windows-версии</p><h2>Тарифы только для администраторов</h2><p>Игроки, тренеры и вратари пользуются веб-версией и Android-приложением бесплатно.</p></div><div className="pricing-grid">{plans.map(([name,price,badge,items],index)=><article className={`price-card ${index===1?"featured-plan":""}`} key={name as string}><span className="plan-badge">{badge as string}</span><div className="puck-mini"/><h3>{name as string}</h3><strong>{price as string}</strong><ul>{(items as string[]).map(item=><li key={item}>✓ {item}</li>)}</ul><a className={index===1?"button button-pink":"button button-outline"} href="mailto:office@codberry.ru?subject=Тариф XM 5.1">Выбрать тариф</a></article>)}</div></section>
